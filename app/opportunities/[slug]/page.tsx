...
import { ArrowLeft, ArrowRight, BadgeCheck, Flame, Loader2, MapPin, Target, Users } from 'lucide-react';
...
export default function OpportunityPage(){
  ...
  useEffect(()=>{(async()=>{
    ...
    setUser(current);
    if(current.role!=='sales'){setError('Company accounts cannot fire on opportunities.');setChecking(false);return;}
    try{
      const token=await getIdToken();
      const profileRes=await fetch('/api/profile',{headers:{Authorization:`Bearer ${token}`}});
      const profileBody=await profileRes.json();
      if(!profileRes.ok||!profileBody.profile?.profileComplete){window.location.href=`/onboarding?next=${encodeURIComponent(`/opportunities/${params.slug}`)}`;return;}
      const fireRes=await fetch(`/api/fires/${params.slug}`,{headers:{Authorization:`Bearer ${token}`}});
      if(fireRes.ok){const body=await fireRes.json();setFired(Boolean(body.fire));}
    }catch(err){setError(err instanceof Error?err.message:'Unable to load this opportunity.');}
    setChecking(false);
  })()},[params.slug]);
  ...
  async function fireNow(){
    ...
    try{
      const token=await getIdToken();
      const res=await fetch(`/api/fires/${params.slug}`,{method:'POST',headers:{Authorization:`Bearer ${token}`}});
      const body=await res.json();
      if(!res.ok)throw new Error(body.error||'Unable to fire on this deal.');
      setFired(true);
    }catch(err){setError(err instanceof Error?err.message:'Unable to fire on this deal.');}
