...
export async function createAgreement(req:Request,fireId:string){
 ...
 const item={agreementId,fireId,opportunityId:fire.opportunityId,companyId:identity.userId,repId:fire.repId,status:'AWAITING_REP',version:1,terms:{companyName:company?.companyName||op.companyName,repName:`${fire.repSnapshot?.firstName||''} ${fire.repSnapshot?.lastName||''}`.trim(),campaign:op.title,territory:op.territory,commission:op.commission,basis:op.basis,paymentDays:30,leadProtectionDays:90,tailDays:60,leadChallengeHours:48},companyAcceptedAt:now,createdAt:now,updatedAt:now};
...
export async function getAgreementFor(req:Request,agreementId:string){
 const identity=await identityFromRequest(req),key=normalizeAgreementId(agreementId);
...
export async function acceptAgreement(req:Request,agreementId:string){
 const identity=await identityFromRequest(req),key=normalizeAgreementId(agreementId),a:any=await getAgreementFor(req,key);
...
export async function transitionLead(req:Request,leadId:string,body:any){
 ...
 if(action==='QUALIFY'&&rep&&lead.status==='REGISTERED')next='QUALIFIED';
   ...
   const a:any=await getAgreementFor(req,lead.agreementId),raw=String(a.terms.commission||''),percent=raw.includes('%'),commission=percent?Math.round((lead.contractValue||0)*(money(raw)/100)*100)/100:money(raw);
   next='COMMISSION_DUE';extra.commissionDue=commission;extra.commissionDueAt=now;extra.paymentDueAt=new Date(Date.now()+(a.terms.paymentDays||30)*86400000).toISOString();
 }
 ...
 const values:any={':s':next,':u':now,':timeline':timeline};let update='SET #s=:s, updatedAt=:u, timeline=:timeline';
 for(const[k,v]of Object.entries(extra)){update+=`, ${k}=:${k}`;values[`:${k}`]=v;}
