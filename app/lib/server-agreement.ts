...
export async function createAgreement(req:Request,fireId:string){
 ...
 const company:any=await getProfile(identity.userId);
 const now=new Date().toISOString();
 const agreementId=fireId;
...
export async function listAgreements(req:Request){
 const identity=await identityFromRequest(req);
 const r=await ddb.send(new ScanCommand({TableName:agreements}));
 return(r.Items||[]).filter((a:any)=>a.repId===identity.userId||a.companyId===identity.userId);
}
...
export async function listLeads(req:Request){
 const identity=await identityFromRequest(req);
 const r=await ddb.send(new ScanCommand({TableName:leads}));
 return(r.Items||[]).filter((l:any)=>l.repId===identity.userId||l.companyId===identity.userId).sort((a:any,b:any)=>b.createdAt.localeCompare(a.createdAt));
}
